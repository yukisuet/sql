...
```
Type 1 (Overwrite): Only the most current address is stored, and previous addresses are lost when a new one is provided.
Type 2 (Add New Record): A new record is created for each address change, preserving the full history of the customer's address timeline. 
```
